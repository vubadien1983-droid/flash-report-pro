...
export default function OpsFindingsWorkspace({
  ...
  canImport = false,       // Import Excel + New section on the Summary tab
  importLocked = false,    // share link: true until the MASTER password is entered
  onRequestImportUnlock,   // (then) => void — ask for the master password, then run `then`
        ...
        <div className={`space-y-2 ${pin ? 'flex-1 min-h-0 overflow-y-auto' : ''}`}>
          ...
          <div className="bg-white rounded-xl border border-slate-200 px-2.5 py-2 flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-1.5">
              ...
              {canImport && !readOnly && (
                <>
                  <button type="button" onClick={() => (importLocked ? onRequestImportUnlock?.(() => setNewSection('')) : setNewSection(''))}
                  ...
                  <button type="button"
                    onClick={() => (importLocked ? onRequestImportUnlock?.(() => fileRef.current?.click()) : fileRef.current?.click())}
                    title={importLocked ? 'Import needs the master password' : 'Import the Excel file'}
                  ...
                  <tr key={s.section} onClick={() => setTab(keyOf(s.section))}
                    ...
                    <td className="px-3 py-2 border-b border-slate-100 font-bold">
                      <span className="inline-flex items-center gap-1.5">
                        {isUnlocked(s.letter) && !readOnly ? <Unlock className="w-3 h-3 text-emerald-600" /> : <Lock className="w-3 h-3 text-slate-400" />}
                        {s.section}
                      </span>
                    </td>
