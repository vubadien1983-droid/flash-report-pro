...
import {
  Link as LinkIcon, Copy, Check, ExternalLink, QrCode, X, Share2, Sparkles, Smartphone
} from 'lucide-react';

export default function ShareModal({ isOpen, shareUrl, reportTitle, onClose }) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: reportTitle || 'Flash Inspection Report',
          text: `Flash Inspection Report: ${reportTitle || ''} (Block B - EPC#1)`,
          url: shareUrl
        });
      } catch (err) {
  ...
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(shareUrl)}&margin=4`;
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-5 md:p-6 border border-slate-200">
        ...
        <div className="flex items-start justify-between mb-4 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm md:text-base font-bold text-slate-900">Share Flash Report Link</h3>
        ...
        <div className="mb-4 p-2.5 bg-slate-50 border border-slate-200/80 rounded-xl flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-slate-800 truncate">
            {reportTitle || 'Untitled Flash Report'}
          </span>
        </div>
        ...
        {/* QR Code & Mobile Scan Area */}
        <div className="mb-5 p-4 bg-slate-50/80 border border-slate-200/80 rounded-2xl flex flex-col sm:flex-row items-center gap-4">
          <div className="w-28 h-28 bg-white p-1.5 rounded-xl border border-slate-200 shadow-xs flex items-center justify-center flex-shrink-0">
            <img
              src={qrCodeUrl}
              alt="Report QR Code"
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed mb-2.5">
              Open your phone camera to scan and view this report on mobile immediately without typing links.
            </p>
